...
# -------------------------------------------------------
MODEL_PATH = r"C:\Users\adyab\Downloads\siglip-amazonbin-epoch5_model_gpt"
...
print("[INFO] Building product table...")

for meta_file in os.listdir(META_DIR):
    if meta_file.lower().endswith(".json"):
        try:
...
# -------------------------------------------------------
# GET QUANTITY FOR PRODUCT IN IMAGE
# -------------------------------------------------------
...
def search(product_name, asin, user_qty):
    ...
    if user_qty > qty_in_bin:
        warning_msg = (
            ...
            f"⚠ WARNING: Requested quantity ({user_qty}) is greater than what the bin contains ({qty_in_bin})."
            f"</div>"
        )
...
# -------------------------------------------------------
# ROW SELECTION HANDLER (correct for Gradio 6)
# -------------------------------------------------------
def on_select(evt: gr.SelectData):
    row_idx = evt.index[0]
    try:
        name, asin = GLOBAL_TABLE[row_idx]
        return name, asin
    except:
        return "", ""
...
# UI
# -------------------------------------------------------
with gr.Blocks(title="Product Search (Gradio 6 Compatible)") as demo:

    gr.Markdown("## 🔍 Searchable Product Table (Click a Row to Select Product)")
